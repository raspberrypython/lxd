...
func (c *remoteCmd) usage() string {
	return gettext.Gettext(
		"Manage remote LXD servers.\n" +
			"\n" +
			"lxc remote add <name> <url> [--password=PASSWORD]  Add the remote <name> at <url>.\n" +
...
func addServer(config *lxd.Config, server string, addr string, password string) error {
	var r_scheme string
	var r_host string
	var r_port string
...
func (c *remoteCmd) run(config *lxd.Config, args []string) error {
	...
	case "add":
		...
		err := addServer(config, args[1], args[2], c.password)
		if err != nil {
